...
<body class="bg-white min-h-screen flex flex-col overflow-hidden">
    ...
    <main class="relative flex-grow w-full flex flex-col">
        ...
        <div class="absolute inset-0 z-0">
            <!-- Gambar Desktop -->
             @if(strtolower(session('data_pendaftar.domisili')) == 'makassar')
                <img src="{{ asset('storage/makassar.PNG') }}" 
                     alt="Desktop" 
                     class="hidden md:block w-full h-full object-cover object-top">

                <!-- Gambar HP -->
                <img src="{{ asset('storage/phone_makassar.jpeg') }}" 
                     alt="Mobile" 
                     class="block md:hidden w-full h-full object-cover object-top">
             @else
                <img src="{{ asset('storage/desktop_lainnya.jpeg') }}" 
                     alt="Desktop" 
                     class="hidden md:block w-full h-full object-cover object-top">

                <!-- Gambar HP -->
                <img src="{{ asset('storage/phone_lainnya.jpeg') }}" 
                     alt="Mobile" 
                     class="block md:hidden w-full h-full object-cover object-top">
            @endif
        ...
        <div class="relative z-10 w-[90%] md:w-[450px] bg-white/95 backdrop-blur-sm p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl mt-8 md:mt-20 mx-auto md:mr-24 md:ml-auto transition-all">
            ...
            <p class="text-gray-700 leading-relaxed text-sm md:text-base font-medium">
                @if(strtolower(session('data_pendaftar.domisili')) == 'makassar')
                    {{-- Teks Khusus Makassar --}}
                    Anda adalah <b>pegawai</b> di yang bekerja di <b>area perkantoran</b> sekitar jalan <b>A.P. Pettarani</b>. Karena <b>jadwal kerja</b> yang padat, Anda mengandalkan <b>Go-Food</b> minimal <b>6x sebulan</b> untuk makan siang.
                @else
                    {{-- Teks untuk Lainnya / Selain Makassar --}}
                    Anda adalah <b>pegawai</b> di yang bekerja di <b>area perkantoran</b>. Karena <b>jadwal kerja</b> yang padat, Anda mengandalkan <b>Go-Food</b> minimal <b>6x sebulan</b> untuk makan siang.
                @endif    
            </p>
        </div>
